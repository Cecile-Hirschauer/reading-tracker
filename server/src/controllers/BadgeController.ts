...
export class BadgeController {
  ...
  static async getAllBadges(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await BadgeService.getAllBadges(page, limit);
   ...
   */
  static async getBadgeById(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  static async getBadgesByRarity(req: Request, res: Response): Promise<void> {
    try {
      ...
      const badges = await BadgeService.getBadgesByRarity(rarity as BadgeRarity);
   ...
   */
  static async checkAvailableBadges(req: Request, res: Response): Promise<void> {
    try {
      const userId = (req as any).user?.userId;
      ...
      const availableBadges = await BadgeService.checkAvailableBadges(userId);
   ...
   */
  static async getRecentlyUnlockedBadges(req: Request, res: Response): Promise<void> {
    try {
      const limit = parseInt(req.query.limit as string) || 10;
      
      const recentBadges = await BadgeService.getRecentlyUnlockedBadges(limit);
   ...
   */
  static async getBadgeLeaderboard(req: Request, res: Response): Promise<void> {
    try {
      const limit = parseInt(req.query.limit as string) || 10;
  ...
  static async createBadge(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Vérifier que la rareté est valide
      if (!Object.values(BadgeRarity).includes(badgeData.rarity)) {
        res.status(400).json({
          success: false,
          message: 'Rareté invalide',
        });
  ...
  static async updateBadge(req: Request, res: Response): Promise<void> {
    try {
      ...
      if (updateData.rarity && !Object.values(BadgeRarity).includes(updateData.rarity)) {
        res.status(400).json({
          success: false,
          message: 'Rareté invalide',
        });
   ...
   */
  static async assignBadgeToUser(req: Request, res: Response): Promise<void> {
    try {
      const { badgeId, userId } = req.params;
      
      await BadgeService.assignBadgeToUser(userId, badgeId);
   ...
   */
  static async removeBadgeFromUser(req: Request, res: Response): Promise<void> {
    try {
      const { badgeId, userId } = req.params;
   ...
   */
  static async getUserBadgesById(req: Request, res: Response): Promise<void> {
    try {
