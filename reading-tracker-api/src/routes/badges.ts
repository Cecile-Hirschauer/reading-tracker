...
import { PrismaClient } from '@prisma/client';
import { authenticateToken } from '../middleware/auth';
import { getAllBadgeDefinitions, getBadgeProgress } from '../utils/gameLogic';
...
// Get badge progress (earned + progress towards unearned)
router.get('/progress', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    console.error('Erreur lors de la récupération du progrès des badges:', error);
    res.status(500).json({ error: 'Erreur interne du serveur' });
  }
});
  ...
  try {
    ...
    if (!badge) {
      res.status(404).json({
        ...
        message: 'Ce badge n\'existe pas ou ne vous appartient pas.'
      });
      return;
    }
...
router.get('/stats/overview', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const totalPossibleBadges = getAllBadgeDefinitions().length;
    ...
    const completionPercentage = Math.round((totalBadges / totalPossibleBadges) * 100);

    res.json({
      stats: {
        totalEarned: totalBadges,
        totalPossible: totalPossibleBadges,
        completionPercentage,
        remaining: totalPossibleBadges - totalBadges
      },
      recentBadges
    });
  } catch (error) {
